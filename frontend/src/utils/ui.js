...
const renderActionMenu = (highBet, players, activePlayerIndex, phase, inputBet,changeSliderInputFn) => {
	...
	return(
		(phase === 'betting1' || phase === 'betting2' || phase === 'betting3' || phase === 'betting4') ? (players[activePlayerIndex].robot) ? (<h4> {`Current Move: ${players[activePlayerIndex].name}`}</h4>) : (
			...
			<div className='betInputDiv'>
            	<input className='betInput'value={inputBet} onChange={(e) => changeSliderInputFn(e.target.value,min,max)} />
          	</div>
			</React.Fragment>
		) : null
	)
